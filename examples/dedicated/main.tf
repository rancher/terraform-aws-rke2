...
locals {
  ...
  prefix              = "tf-aws-rke2-ded-${substr(local.identifier, 0, 4)}" # this can be anything you want, it makes it marginally easier to find in AWS
  ...
  names                 = { for i in range(local.cluster_size) : tostring(i) => "${local.prefix}-${substr(md5(uuidv5("dns", tostring(i))), 0, 4)}" }
