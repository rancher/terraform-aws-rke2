...
output "ssh" {
  value = merge(
    ...
    { for i in range(local.server_count, (local.server_count + local.local.agent_count)) : local.names[i] => "${local.username}@${module.Agents[local.names[i]].server_public_ip}" }
  )
}